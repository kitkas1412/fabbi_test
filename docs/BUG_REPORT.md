...
| FE-005 | Pagination | Frontend requests 10,000 Todos by default | Medium | `frontend/src/features/todos/api/todos.ts::useTodos` | Fixed | `<pending commit>` | Default page size is an explicit 100, matching the backend maximum; Node regression, lint, and build pass |
...
| Frontend unit regression | `cd frontend && npm test` | Pass | Pass: 4/4 query-isolation, session-cleanup, auth-401 handling, and bounded-page-size tests | `2026-09-18` | FE-001/FE-002/FE-004/FE-005 regression coverage; Node 20-compatible runner |
