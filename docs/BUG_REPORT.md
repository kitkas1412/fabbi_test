...
| Tier 2A: at least three backend critical scenarios | `backend/tests/` | 25/25 pass, including refresh rotation/revocation, security/correctness/cache, required-secret, and deterministic-order regressions | Complete |
...
| Backend tests | `docker compose run --rm --no-deps backend pytest tests/ -v` | Pass | Pass in fresh non-root Python 3.12.14 image: 25/25 in 5.09s; 3 deprecation warnings | `2026-09-18` | Includes refresh rotation/revocation, required-secret, and deterministic-order regressions; warnings linked to `TEST-003`, `CONFIG-001`, and `DEP-002` |
