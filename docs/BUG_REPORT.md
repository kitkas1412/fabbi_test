...
| Tier 2A: at least three backend critical scenarios | `backend/tests/` | 26/26 pass, including refresh rotation/revocation, login enumeration, security/correctness/cache, required-secret, and deterministic-order regressions | Complete |
...
| Backend tests | `docker compose run --rm --no-deps backend pytest tests/ -v` | Pass | Pass in fresh non-root Python 3.12.14 image: 26/26; 3 deprecation warnings | `2026-09-18` | Includes refresh rotation/revocation, login-enumeration, required-secret, and deterministic-order regressions; warnings linked to `TEST-003`, `CONFIG-001`, and `DEP-002` |
