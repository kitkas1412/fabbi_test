...
| Tier 2A: at least three backend critical scenarios | `backend/tests/` | 28/28 pass, including refresh rotation/revocation, login enumeration, security/correctness/cache, required-secret, and deterministic-order regressions | Complete |
...
| Backend tests | `docker compose run --rm --no-deps backend pytest tests/ -v` | Pass | Pass in fresh non-root Python 3.12.14 image: 28/28; 3 deprecation warnings | `2026-09-18` | Includes refresh rotation/revocation, login-enumeration, cache commit-ordering, required-secret, and deterministic-order regressions; warnings linked to `TEST-003`, `CONFIG-001`, and `DEP-002` |
...
| Migration upgrade/downgrade | `alembic upgrade head`; `alembic current` against PostgreSQL | Pass | Upgrade/current pass at `c3d5e7f9a1b2 (head)`; performance database downgrade/re-upgrade also passed | `2026-09-18` | Migration was applied during backend startup; concurrent-index rollback evidence is in `docs/PERFORMANCE_REPORT.md` |
