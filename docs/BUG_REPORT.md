...
| TEST-001 | Coverage | Existing tests cover happy paths but not security boundaries | High | `backend/tests/` | Verified | `cea96fd`, `5816658` | Suite now covers expiry, token type, cross-user CRUD, cache isolation/invalidation, partial updates, required secrets, and stable ordering; 23/23 pass |
...
| Tier 2A: at least three backend critical scenarios | `backend/tests/` | 23/23 pass, including security/correctness/cache, required-secret, and deterministic-order regressions | Complete |
